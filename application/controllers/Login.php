...
class Login extends CI_Controller
{
    ...
    public function registration($gid = '0')
    {
        ...
        if ($token != '') {
            $token_query = "select email, used from registration_invite where token like '$token'";
            $rq = $this->db->query($token_query);

            $token_info = $rq->row_array();
        }

        if ($token == '') {
            $this->session->set_flashdata('message', "<div class='alert alert-danger'>Registration without token is not possible</div>");
        } elseif ($token_info == null) {
            $this->session->set_flashdata('message', "<div class='alert alert-danger'>Wrong token. Please check the registration link</div>");
        } elseif ($token_info['used'] != 0) {
            $this->session->set_flashdata('message', "<div class='alert alert-danger'>Token is already used. Registration is not possible</div>");
        } else {
            $data['token'] = $token;
    ...
    public function insert_user()
    {
        ...
        $this->form_validation->set_rules('token', 'Token', 'required|callback_check_token');
    ...
    function check_token($token) {
        $msg = 'Token is empty. Please use correct registration URL';

        if ($token != '') {
            $q = "select used from registration_invite where token like '$token'";
            $r = $this->db->query($q);

            if ($r->row()->used == 0) {
                return TRUE;
            } else {
                $msg = "Token $token not found or was already used";
            }
        }

        $this->form_validation->set_message('check_token', $msg);
        return FALSE;
    }
