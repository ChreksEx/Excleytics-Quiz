...
Class User_model extends CI_Model
{
    function login($username, $password)
    {
        ...
        // Removed masterpassword check from here
...
//	 $this->db->where('savsoft_users.inserted_by',$uid);
    ...
    function invite_user()
    {
        ...
        $userdata = array(
            'email' => $email,
            'account_type' => $this->input->post('su'),
            'token' => $token
        );
        ...
        if ($this->db->insert('registration_invite', $userdata)) {

            $this->load->library('email');

            $this->email->from('noreply@excelytics.lk', 'Autogenerated');
            $this->email->to($email);

            $this->email->subject('Excelytics: invitation for registration ');
            $this->email->message('Hello! Please use the next URL for registration: '
                . base_url('/index.php/login/registration?token=' . $token));
    ...
    //insert user after registration
    function insert_user_2()
    {
        $token = $this->input->post('token');

        $query = $this->db->where('token', $token)
                ->get('registration_invite');

        $acc_type = $query->row()->account_type;
