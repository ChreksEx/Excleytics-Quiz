...
class Result extends CI_Controller {
	...
	function view_result($rid){
        ...
        // for select_letters
        $letters = $this->result_model->get_letters(array_column($data['saved_answers'], 'q_option'));
        $letters_counts = array_count_values(array_column($letters, 'q_option_match'));
        arsort($letters_counts);

        $data['letters_counts'] = array_slice($letters_counts, 0, 6);
