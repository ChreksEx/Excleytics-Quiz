...
class Qbank extends CI_Controller
{
    ...
    public function pre_new_question()
    {
        ...
        if ($this->input->post('question_type')) {
            ...
            if ($this->input->post('question_type') == '6') {
                ...
                redirect('qbank/new_question_6/' . $nop . '/' . $para);
            }
    ...
    public function new_question_6($nop = '2', $para = '0')
    {
        ...
        if ($this->input->post('question')) {
            ...
            $is_letters_and_options = true;
            foreach (array_merge($this->input->post('letter'), $this->input->post('option')) as $item) {
                if($item == '') {
                    $is_letters_and_options = false;
                    break;
                }
            }

            if ($this->qbank_model->insert_question_6() && $is_letters_and_options) {
                $this->session->set_flashdata('message', "<div class='alert alert-success'>" . $this->lang->line('data_added_successfully') . " </div>");
            } else {
                $this->session->set_flashdata('message', "<div class='alert alert-danger'>" . $this->lang->line('error_to_add_data') . " </div>");
            }
